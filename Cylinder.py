...
def GetPointsonCube(height):
    density = 10
    zs = np.linspace(0, height, density)
...
def Transformto2D(points, focal_dist):
    ...
    for point in points:
        if point[2] != 0:
            xt = point[0]*focal_dist/(point[2] + 30) 
            yt = point[1]*focal_dist/(point[2] + 30)
        else:
            xt = point[0] 
            yt = point[1] 
...
# generate 3D points
points = GetPointsonCube(height=10)
