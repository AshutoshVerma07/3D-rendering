...
# returns a point of the donut based on the parametric equation and parameter given 
def PointonDonut(R,r,u,v):
    x = (R+r*math.sin(u))*math.cos(v)
    y = -(R+r*math.sin(u))*math.sin(v)
    z = r*math.cos(u)
    return (x,y,z)

# return a list of points on the surface of donut given it's innner and outer radius
def GetPointsonDonut(R,r):
    u_range = 20
    v_range = 30
    u_list = np.linspace(0, 2*np.pi, u_range)
    v_list = np.linspace(0, 2*np.pi, v_range)
    points = []
    for v in v_list:
        for u in u_list:
            points.append(PointonDonut(R, r, u, v))
    return points
...
# generate 3D points
points = GetPointsonDonut(R=10, r=5)
