...
def GetPointsonHelix(h, p, R, r):
    points = []
    a = h/p * 2*np.pi
    rn = np.linspace(0, 2*np.pi, 6)
    Rn = np.linspace(0, a, 100)
    zw = p/20
    for i in Rn:
        for j in rn:
            x = R + r*np.sin(j)
            y = 0
            z = r*np.cos(j) - h/4
            x1 = x*np.cos(i) - y*np.sin(i)
            y1 = x*np.sin(i) + y*np.cos(i)
            points.append((x1,y1,z + i*zw))
...
def Transformto2D(points, focal_dist):
    ...
    for point in points:
        if point[2] != 0:
            xt = point[0]*focal_dist/(point[2] - 80) 
            yt = point[1]*focal_dist/(point[2] - 80)
        else:
            xt = point[0] 
            yt = point[1] 
...
# generate 3D points
points = GetPointsonHelix(h=100, p=40, R = 10, r = 2)
...
while running:
    ...
    rtpoints = Transformto2D(rpoints, focal_dist=600)
